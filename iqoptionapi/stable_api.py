...
class IQ_Option:
    ...
    def __init__(self, email, password, active_account_type="PRACTICE"):
        ...
        self.subscribed_top_assets = set()
        self.subscribed_commission_changed = set()
        # for digit
    ...
    def re_subscribe_stream(self):
        ...
        try:
            ...
            pass
        try:
            for itype in list(self.subscribed_top_assets):
                self.api.Subscribe_Top_Assets_Updated(itype)
        except:
            pass
        try:
            for itype in list(self.subscribed_commission_changed):
                self.api.Subscribe_Commission_Changed(itype)
        except:
            pass
    ...
    def _wait_until(self, predicate, timeout=None, interval=0.05):
        """Aguarda até predicate() retornar True ou exceder timeout (segundos)."""
    ...
    def connect(self, sms_code=None):
        ...
        if check == True:
            ...
            if not self._wait_until(lambda: self.api.state.balance_id is not None, timeout=self.default_timeout):
                logging.error('connect timeout waiting for balance_id')
                return False, 'Timeout waiting for balance_id'

            self.position_change_all(
                "subscribeMessage", self.api.state.balance_id)
    ...
    def check_connect(self):
        ...
        if not self.api.state.check_websocket_if_connect:
            return False
        else:
            return True
    ...
    def get_financial_information(self, activeId):
        ...
        if not self._wait_until(lambda: self.api.financial_information is not None, timeout=self.default_timeout):
            logging.error('get_financial_information timeout')
            return None
    ...
    def get_leader_board(self, country, from_position, to_position, near_traders_count, user_country_id=0, near_traders_country_count=0, top_country_count=0, top_count=0, top_type=2):
        ...
        if not self._wait_until(lambda: self.api.leaderboard_deals_client is not None, timeout=self.default_timeout):
            logging.error('get_leader_board timeout')
    ...
    def get_profile_ansyc(self):
        if not self._wait_until(lambda: self.api.profile.msg is not None, timeout=self.default_timeout):
            logging.error('get_profile_ansyc timeout')
    ...
    def get_currency(self):
        ...
        for balance in balances_raw["msg"]:
            if balance["id"] == self.api.state.balance_id:
                return balance["currency"]

    def get_balance_id(self):
        return self.api.state.balance_id
    ...
    def get_balances(self):
        ...
        if not self._wait_until(lambda: self.api.balances_raw is not None, timeout=self.default_timeout):
            logging.error('get_balances timeout')
    ...
    def reset_practice_balance(self):
        ...
        if not self._wait_until(lambda: self.api.training_balance_reset_request is not None, timeout=self.default_timeout):
            logging.error('reset_practice_balance timeout')
    ...
    def change_balance(self, Balance_MODE):
        def set_id(b_id):
            if self.api.state.balance_id != None:
                self.position_change_all(
                    "unsubscribeMessage", self.api.state.balance_id)

            self.api.state.balance_id = b_id
    ...
    def subscribe_top_assets_updated(self, instrument_type):
        ...
        self.subscribed_top_assets.add(instrument_type)

    def unsubscribe_top_assets_updated(self, instrument_type):
        self.api.Unsubscribe_Top_Assets_Updated(instrument_type)
        if instrument_type in self.subscribed_top_assets:
            self.subscribed_top_assets.remove(instrument_type)
    ...
    def subscribe_commission_changed(self, instrument_type):
        ...
        self.subscribed_commission_changed.add(instrument_type)

    def unsubscribe_commission_changed(self, instrument_type):
        self.api.Unsubscribe_Commission_Changed(instrument_type)
        if instrument_type in self.subscribed_commission_changed:
            self.subscribed_commission_changed.remove(instrument_type)
    ...
    def start_mood_stream(self, ACTIVES, instrument="turbo-option"):
        ...
        if not self._wait_until(lambda: OP_code.ACTIVES[ACTIVES] in self.api.traders_mood, timeout=self.default_timeout, interval=0.2):
            logging.error('start_mood_stream timeout for %s', ACTIVES)
            return False
        return True
    ...
    def buy_by_raw_expirations(self, price, active, direction, option, expired):
        ...
        while self.api.result == None or id == None:
            ...
            if time.time() - start_t >= 5:
                ...
                return False, None
        # preferir resultado específico do req_id
        success = self.api.wait_result(req_id, timeout=self.default_timeout)
        result_flag = True if success is None else bool(success)
        return result_flag, self.api.buy_multi_option[req_id]["id"]

    def buy(self, price: float, ACTIVES: str, ACTION: str, expirations: int):
        self.api.buy_multi_option = {}
        self.api.buy_successful = None
        # req_id = "buy"
        ...
        # Prefer Event por request_id
        try:
            message = self.api.wait_request(req_id, timeout=self.default_timeout)
            if message:
                payload = message.get("msg", {})
                if "message" in payload:
                    return False, payload.get("message")
                trade_id = payload.get("id")
                # aguarda 'result' específico do req_id, se disponível
                success = self.api.wait_result(req_id, timeout=self.default_timeout)
                result_flag = True if success is None else bool(success)
                return result_flag, trade_id
        except Exception:
            pass
        ...
        trade_id = None
        self.api.result = None
        while self.api.result == None or trade_id == None:
            try:
                if "message" in self.api.buy_multi_option[req_id].keys():
                    return False, self.api.buy_multi_option[req_id]["message"]
            except:
                pass
            try:
                trade_id = self.api.buy_multi_option[req_id]["id"]
            except:
                pass
            if time.time() - start_t >= self.default_timeout:
                logging.error('buy timeout after %.1fs', self.default_timeout)
                return False, None

        return self.api.result, trade_id
...
# __________________for Digital___________________
    ...
    def get_digital_underlying_list_data(self):
        ...
        while self.api.underlying_list_data == None:
            if time.time() - start_t >= max(self.default_timeout, 30):
                logging.error('**warning** get_digital_underlying_list_data late')
                return None
            time.sleep(0.05)
    ...
    def get_instrument_quites_generated_data(self, ACTIVE, duration):
        if not self._wait_until(lambda: self.api.instrument_quotes_generated_raw_data[ACTIVE][duration * 60] != {}, timeout=self.default_timeout):
            logging.error('get_instrument_quites_generated_data timeout')
            return {}
        return self.api.instrument_quotes_generated_raw_data[ACTIVE][duration * 60]

    def get_realtime_strike_list(self, ACTIVE, duration):
        if not self._wait_until(lambda: bool(self.api.instrument_quites_generated_data[ACTIVE][duration * 60]), timeout=self.default_timeout):
            logging.error('get_realtime_strike_list timeout waiting for quotes')
            return {}
        """
        ...
        start_t = time.time()
        while ans == {} and (time.time() - start_t) < self.default_timeout:
    ...
    def buy_digital_spot(self, active, amount, action, duration):
        ...
        # Preferir evento por request_id (Fase 2); fallback ao legado
        try:
            message = self.api.wait_request(request_id, timeout=self.default_timeout)
            if message and isinstance(message.get("msg", {}).get("id"), int):
                digital_order_id = message["msg"]["id"]
                return True, digital_order_id
            elif message and message.get("msg", {}).get("message"):
                return False, message["msg"]["message"]
        except Exception:
            pass
    ...
    def buy_digital(self, amount, instrument_id):
        self.api.digital_option_placed_id = None
        request_id = self.api.place_digital_option(instrument_id, amount)
        # Prefer event by request_id first
        try:
            message = self.api.wait_request(request_id, timeout=max(self.default_timeout, 30))
            if message and isinstance(message.get("msg", {}).get("id"), int):
                return True, message["msg"]["id"]
            elif message and message.get("msg", {}).get("message"):
                logging.error('buy_digital error: %s', message["msg"]["message"])
                return False, None
        except Exception:
            pass
        start_t = time.time()
    ...
    def buy_order(self,
                  ...
                  type: str, limit_price=None, stop_price=None,
        ...
        # Tentar obter por request_id primeiro
        if req_id:
            try:
                msg = self.api.wait_request(req_id, timeout=self.default_timeout)
                if msg and isinstance(msg.get("msg", {}).get("id"), int):
                    self.api.buy_order_id = msg["msg"]["id"]
            except Exception:
                pass

        # Fallback legado (aguarda handler preencher buy_order_id)
        start_t = time.time()
        while self.api.buy_order_id == None:
            if time.time() - start_t > self.default_timeout:
                logging.error('buy_order timeout waiting id')
                return False, None
            time.sleep(0.05)

        check, data = self.get_order(self.api.buy_order_id)
        t0 = time.time()
        while data and data.get("status") == "pending_new":
            if time.time() - t0 > self.default_timeout:
                logging.error('buy_order timeout waiting filled/rejected')
                break
            check, data = self.get_order(self.api.buy_order_id)
            time.sleep(0.2)
    ...
    def change_auto_margin_call(self, ID_Name, ID, auto_margin_call):
        ...
        req_id = self.api.change_auto_margin_call(ID_Name, ID, auto_margin_call)
        if req_id:
            try:
                message = self.api.wait_request(req_id, timeout=self.default_timeout)
                if message and message.get("name") == "auto-margin-call-changed":
                    self.api.auto_margin_call_changed_respond = message
            except Exception:
                pass
        if not self._wait_until(lambda: self.api.auto_margin_call_changed_respond is not None, timeout=self.default_timeout):
            return False, None
    ...
    def change_order(self, ID_Name, order_id,
        ...
        if check:
            ...
            # Wait for tpsl-changed
            if req_id:
                try:
                    message = self.api.wait_request(req_id, timeout=self.default_timeout)
                    if message and message.get("name") == "tpsl-changed":
            ...
            if not self._wait_until(lambda: self.api.tpsl_changed_respond is not None, timeout=self.default_timeout):
                return False, None
    ...
    def get_order(self, buy_order_id):
        ...
        req_id = self.api.get_order(buy_order_id)
        if req_id:
            try:
                message = self.api.wait_request(req_id, timeout=self.default_timeout)
                if message and message.get("name") == "order":
                    self.api.order_data = message
            except Exception:
                pass
        if not self._wait_until(lambda: self.api.order_data is not None, timeout=self.default_timeout):
            logging.error('get_order timeout')
            return False, None
    ...
    def get_pending(self, instrument_type):
        ...
        req_id = self.api.get_pending(instrument_type)
        if req_id:
            try:
                message = self.api.wait_request(req_id, timeout=self.default_timeout)
                if message and message.get("name") == "deferred-orders":
                    self.api.deferred_orders = message
            except Exception:
                pass
        if not self._wait_until(lambda: self.api.deferred_orders is not None, timeout=self.default_timeout):
            logging.error('get_pending timeout')
            return False, None
    ...
    def get_positions(self, instrument_type):
        ...
        req_id = self.api.get_positions(instrument_type)
        if req_id:
            try:
                message = self.api.wait_request(req_id, timeout=self.default_timeout)
                if message and message.get("name") == "positions":
                    self.api.positions = message
            except Exception:
                pass
        if not self._wait_until(lambda: self.api.positions is not None, timeout=self.default_timeout):
            logging.error('get_positions timeout')
            return False, None
    ...
    def get_position(self, buy_order_id):
        ...
        position_id = order_data["position_id"]
        req_id = self.api.get_position(position_id)
        if req_id:
            try:
                message = self.api.wait_request(req_id, timeout=self.default_timeout)
                if message and message.get("name") == "position":
                    self.api.position = message
            except Exception:
                pass
        if not self._wait_until(lambda: self.api.position is not None, timeout=self.default_timeout):
            logging.error('get_position timeout')
            return False, None
    ...
    def get_digital_position_by_position_id(self, position_id):
        self.api.position = None
        req_id = self.api.get_digital_position(position_id)
        if req_id:
            try:
                message = self.api.wait_request(req_id, timeout=self.default_timeout)
                if message and message.get("name") == "position":
                ...
                pass
        if not self._wait_until(lambda: self.api.position is not None, timeout=self.default_timeout):
            logging.error('get_digital_position_by_position_id timeout')
            return None
        return self.api.position

    def get_digital_position(self, order_id):
        self.api.position = None
        if not self._wait_until(lambda: self.get_async_order(order_id)["position-changed"] != {}, timeout=self.default_timeout):
            logging.error('get_digital_position timeout waiting for position-changed')
            return None
        ...
        if req_id:
            try:
                ...
                pass
        if not self._wait_until(lambda: self.api.position is not None, timeout=self.default_timeout):
            logging.error('get_digital_position timeout waiting for position data')
            return None
        return self.api.position
    ...
    def get_position_history(self, instrument_type):
        ...
        if not self._wait_until(lambda: self.api.position_history is not None, timeout=self.default_timeout):
            logging.error('get_position_history timeout')
            return False, None
    ...
    def get_position_history_v2(self, instrument_type, limit, offset, start, end):
        ...
        if not self._wait_until(lambda: self.api.position_history_v2 is not None, timeout=self.default_timeout):
            logging.error('get_position_history_v2 timeout')
            return False, None
    ...
    def get_available_leverages(self, instrument_type, actives=""):
        ...
        if actives == "":
            req_id = self.api.get_available_leverages(instrument_type, "")
        else:
            req_id = self.api.get_available_leverages(
                instrument_type, OP_code.ACTIVES[actives])
        if req_id:
            try:
                message = self.api.wait_request(req_id, timeout=self.default_timeout)
                if message and message.get("name") == "available-leverages":
                    self.api.available_leverages = message
            except Exception:
                pass
        if not self._wait_until(lambda: self.api.available_leverages is not None, timeout=self.default_timeout):
            logging.error('get_available_leverages timeout')
            return False, None
    ...
    def cancel_order(self, buy_order_id):
        ...
        req_id = self.api.cancel_order(buy_order_id)
        if req_id:
            try:
                message = self.api.wait_request(req_id, timeout=self.default_timeout)
                if message and message.get("name") == "order-canceled":
                    self.api.order_canceled = message
            except Exception:
                pass
        if not self._wait_until(lambda: self.api.order_canceled is not None, timeout=self.default_timeout):
            logging.error('cancel_order timeout')
    ...
    def close_position(self, position_id):
        ...
        if data["position_id"] != None:
            ...
            req_id = self.api.close_position(data["position_id"])
            if req_id:
                try:
                    message = self.api.wait_request(req_id, timeout=self.default_timeout)
                    if message and message.get("name") == "position-closed":
                        self.api.close_position_data = message
                except Exception:
                    pass
            if not self._wait_until(lambda: self.api.close_position_data is not None, timeout=self.default_timeout):
                logging.error('close_position timeout')
                return False
    ...
    def close_position_v2(self, position_id):
        if not self._wait_until(lambda: self.get_async_order(position_id) is not None, timeout=self.default_timeout):
            logging.error('close_position_v2 timeout waiting for async order')
            return False
        position_changed = self.get_async_order(position_id)
        self.api.close_position(position_changed["id"])
        if not self._wait_until(lambda: self.api.close_position_data is not None, timeout=self.default_timeout):
            logging.error('close_position_v2 timeout waiting response')
            return False
        if self.api.close_position_data["status"] == 2000:
            return True
        else:
    ...
    def get_user_profile_client(self, user_id):
        ...
        if not self._wait_until(lambda: self.api.user_profile_client is not None, timeout=self.default_timeout):
            logging.error('get_user_profile_client timeout')
            return None
    ...
    def request_leaderboard_userinfo_deals_client(self, user_id, country_id):
        ...
        while True:
            try:
                ...
                pass
            if time.time() - start_t > self.default_timeout:
                logging.error('request_leaderboard_userinfo_deals_client timeout')
                break
    ...
    def get_users_availability(self, user_id):
        ...
        while self.api.users_availability == None:
            if time.time() - start_t > self.default_timeout:
                logging.error('get_users_availability timeout')
                break
    ...
    def get_digital_payout(self, active, seconds=0):
        ...
        start = time.time()
        limit = seconds if seconds else self.default_timeout
        while self.api.digital_payout is None:
            if int(time.time() - start) > limit:
                break
    ...
    def buy_digital_spot_v2(self, active, amount, action, duration):
        ...
        # Preferir sincronização por request_id
        try:
            message = self.api.wait_request(request_id, timeout=self.default_timeout)
            if message and isinstance(message.get("msg", {}).get("id"), int):
                return True, message["msg"]["id"]
