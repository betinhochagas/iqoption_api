...
class WebsocketClient(object):
    ...
    def on_message(self, wss, message: str):  # pylint: disable=unused-argument
        ...
        # Resolve pending requests by request_id (Phase 2)
        try:
            ...
            if req_id is not None:
                if name == "result":
                    try:
                        self.api.set_result(req_id, message.get("msg", {}).get("success"))
                    except Exception:
                        pass
                if name in resolvable:
                    self.api.resolve_pending(req_id, message)
        except Exception:
            pass
